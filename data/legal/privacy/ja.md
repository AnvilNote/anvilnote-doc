...
AnvilNoteは**ローカルファースト**の設計思想を採用しています。**基本的な編集と文書の書き出しはお客様の端末上で動作します。AIスマートモードは任意機能であり、利用にはインターネット接続とお客様自身のOpenAI APIキーが必要です。** お客様が第三者サービスへの接続を必要とする機能を自ら有効化しない限り、お客様の文書内容は原則としてお客様ご自身の端末にのみ保存され、AnvilNoteのメンテナーが管理するサーバーに送信されることはありません。
...
**AnvilNoteの公開ウェブサイトは、製品情報、法的情報及びデスクトップ版のダウンロード情報のみを提供します。文書エディタ、アプリケーション設定及びAIスマートモードは提供しません。**
...
* 文書の内容；
* 下書き；
* 文書の版管理履歴（バージョン履歴）及び端末内に保存されるAIスマートモードの会話履歴；
* インポート又は添付したファイル；
* ユーザー設定；
* テンプレート、フォントその他機能の実行に必要なローカルデータ；
* デスクトップ版で保存を選択したAIキープロファイルを管理するための暗号化されたローカルメタデータ；
...
AIスマートモードをご利用になる際、AnvilNoteはお客様が選択した機能に応じて、**お客様の端末上で動作する信頼されたアプリケーションコンポーネント**を通じ、次のデータをOpenAIに送信することがあります。

* お客様が入力した指示、質問又はプロンプトのテキスト；
* お客様が自ら選択又は指定した文書の内容；
* お客様が要求した操作を完了するために必要なコンテキスト；
* 当該機能が添付ファイルに対応している場合、お客様が自ら添付又は指定したファイルから端末内で抽出されたテキスト；
* その他お客様が明示的にAIモデルによる処理のために提供することを選択したデータ。

**AnvilNoteは、お客様の操作又は選択を経ずに、お客様の端末内にある文書全体を自発的にOpenAIへ送信することはありません。** 添付ファイルを追加しただけで、無関係な文書又はファイルを送信する許可になることもありません。

#### 4.3 APIキー

お客様が設定したOpenAIのAPIキーは、お客様の端末内にのみ保存されます。

* ブラウザ互換の開発実行環境では、原則としてブラウザのセッション又はその他のローカルストレージに保存されます。公開ウェブサイトはAIスマートモードを提供しません；
* デスクトップ版では、利用可能な場合にオペレーティングシステムの安全な保存機能でキーを暗号化し、選択したキープロファイルの利用に必要な暗号化済みローカル記録のみを保存します。安全な保存機能を利用できない場合、AnvilNoteはキーを永続保存せず、現在のセッション中にのみ保持することがあります。

AIスマートモードをご利用になる際、APIキーは、OpenAIへのリクエストを認可するためにローカルの信頼されたアプリケーション境界で使用されます。**APIキーがAnvilNoteプロジェクトのメンテナーが管理するサーバーに送信されること、又はメンテナーによって保管されることはありません。** 設定画面には保存済みキープロファイルの限定されたマスク済み識別子のみが表示され、保存後に完全なキーは表示されません。

お客様はAPIキーを適切に管理する責任を負い、キーの漏洩、無断使用、API費用の発生又は第三者による不正アクセスから生じるリスクをご自身で負担するものとします。

保存済みキープロファイルはAIスマートモードの設定から削除又は無効化できます。削除すると対応する暗号化済みローカル記録は削除されますが、OpenAI側の元のキーは失効しません。キーの漏洩が疑われる場合は、OpenAIアカウントでも当該キーを失効させてください。
...
#### 4.5 AIスマートモードの結果に対するお客様の管理

AIスマートモードは、構造化された文書の下書き又は選択テキストの変更案を、確認のために返します。**お客様の確認なしに文書を静かに置き換えることはありません。** お客様は、挿入、置換、承認、拒否、再生成又は利用可能な結果の取り消しを選択できます。デスクトップ版に保存される会話及び下書きはローカルアプリケーションデータであり、アプリケーション又は関連するローカルアプリケーションデータを削除して消去できます。
...
本ポリシー、AnvilNoteのデータ処理方法、又は個人情報に関する権利の行使について疑問がある場合、[contact@anvilnote.org](mailto:contact@anvilnote.org) 又は AnvilNoteのGitHubプロジェクトページを通じてメンテナーにお問い合わせいただけます。
